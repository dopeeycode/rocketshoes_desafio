...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      const productAlredyInCard = cart.find(product => product.id === productId)

      if(!productAlredyInCard) {
      ...
      if(productAlredyInCard) {
        ...
        if(stock.amount > productAlredyInCard.amount) {
  ...
  const updateProductAmount = async ({
    ...
    try {
      ...
      if(stockIsNotAvailable) {
        toast.error(`Ops. Temos apenas ${productAmount} produtos no estoque`)
        return
      }
